...
export const loadTokens = async (provider, addresses, dispatch)=>{
    ...
    dispatch({type: 'TOKEN_01_LOADED', token, symbol})

    token = new ethers.Contract(addresses[1],TOKEN_ABI, provider)
    symbol = await token.symbol()
    dispatch({type: 'TOKEN_02_LOADED', token, symbol})
...
/*
    when a event happen inside our contract in this case exchange contract on deposit 
    function : we get a notification from blockchain that the deposit was a success 
    so we can notify our frontend app to reload the page  
*/
export const subscribeToEvent = (exchange, dispatch)=>{
    exchange.on('Deposit', (token, user, amount, balance, event)=>{ 
    /*              about anonymous func :
      our Deposit Event inside Exchange contract have these args and 
                returns the event itself as it last arg
    */       
...
export const loadBalances = async(exchange, tokens, account ,dispatch)=>{
    ...
    dispatch({type: 'TOKEN_01_BALANCE_LOADED', balance} )

    balance = ethers.utils.formatUnits(await exchange.balanceOf(tokens[0].address, account), 18)
    dispatch({type: 'EXCHANGE_TOKEN_01_BALANCE_LOADED', balance})

    balance = ethers.utils.formatUnits(await tokens[1].balanceOf(account), 18)
    dispatch({type: 'TOKEN_02_BALANCE_LOADED', balance} )
